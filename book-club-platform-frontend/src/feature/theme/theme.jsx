  ...
  return (
    <div className="fixed bottom-5 right-5">
      <Button
        ...
        className={`p-2 transition-all duration-300 hover:scale-110  ${!isLightTheme() ? "bg-yellow-700" : "bg-yellow-300"}`} 
      >
        {isLightTheme() ? (
          <Moon className="w-8 h-8 text-gray-800 dark:text-gray-200 transition-all duration-300 " />
        ) : (
          <Sun className="w-8 h-8 text-gray-800 transition-all duration-300" />
        )}
      </Button>
    </div>
  );
